...
impl EventListenerProvider for LoggingEventListener {
    ...
    async fn on_admin_event(&self, event: &AdminEvent, _include_representation: bool) -> Result<()> {
        tracing::info!(
            ...
            event.auth_details.username.as_deref().unwrap_or(&event.auth_details.user_id),
...
impl EventListenerProvider for EmailEventListener {
    async fn on_event(&self, event: &Event) -> Result<()> {
        ...
        match event.event_type {
            crate::models::events::EventType::UserDisabledByPermanentLockout => {
                ...
                tracing::info!("Would send email notification: User permanently locked out: {}", event.user_id.as_deref().unwrap_or("unknown"));
            }
            crate::models::events::EventType::UserDisabledByTemporaryLockout => {
                // Send email to admin about temporary lockout
                tracing::info!("Would send email notification: User temporarily locked out: {}", event.user_id.as_deref().unwrap_or("unknown"));
            }
            _ => {
    ...
    async fn on_admin_event(&self, event: &AdminEvent, _include_representation: bool) -> Result<()> {
        ...
        match (&event.operation_type, &event.resource_type) {
            (crate::models::events::OperationType::Delete, crate::models::events::ResourceType::User) => {
                tracing::info!("Would send email notification: User deleted by admin {}", event.auth_details.username.as_deref().unwrap_or("unknown"));
            }
            (crate::models::events::OperationType::Create, crate::models::events::ResourceType::Realm) => {
                tracing::info!("Would send email notification: New realm created by admin {}", event.auth_details.username.as_deref().unwrap_or("unknown"));
            }
            _ => {
