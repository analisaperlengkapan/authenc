...
pub async fn sync_env_configs_to_db(db: &Database, configs: &[(SocialProvider, OAuthConfig)]) -> Result<()> {
    ...
    let realm_row = db.query_opt(realm_query, &[]).await.map_err(|e| {
        crate::error::AuthencError::database(format!("Failed to query master realm: {}", e))
    })?;
    ...
    for (provider, config) in configs {
        ...
        let existing = db.query_opt(check_query, &[&realm_id, &alias]).await.map_err(|e| {
            crate::error::AuthencError::database(format!("Failed to check existing provider: {}", e))
        })?;
        ...
        if existing.is_some() {
            ...
            db.execute(update_query, &[
                ...
                crate::error::AuthencError::database(format!("Failed to update provider config: {}", e))
            })?;
        } else {
            // Insert
            let insert_query = r#"
            ...
            db.execute(insert_query, &[
                ...
                crate::error::AuthencError::database(format!("Failed to insert provider config: {}", e))
            })?;
        }
    }
