...
pub trait SocialAccountStoreTrait: Send + Sync {
    ...
    async fn get_social_account(&self, account_id: Uuid) -> Result<Option<SocialAccount>, AuthencError>;

    /// Get social accounts for a user
    async fn get_user_social_accounts(&self, user_id: Uuid) -> Result<Vec<SocialAccount>, AuthencError>;

    /// Get social account by provider and provider user ID
    async fn get_social_account_by_provider(&self, provider: &SocialProvider, provider_user_id: &str) -> Result<Option<SocialAccount>, AuthencError>;

    /// Check if user has social account linked for provider
    async fn has_social_account(&self, user_id: Uuid, provider: &SocialProvider) -> Result<bool, AuthencError>;

    /// Create a new social account link
    async fn add_social_account(&self, user_id: Uuid, request: CreateSocialAccountRequest) -> Result<SocialAccount, AuthencError>;

    /// Update social account information
    async fn update_social_account(&self, account_id: Uuid, request: CreateSocialAccountRequest) -> Result<SocialAccount, AuthencError>;
    ...
    async fn remove_social_account_by_provider(&self, user_id: Uuid, provider: &SocialProvider) -> Result<(), AuthencError>;
}
...
impl SocialAccountStoreTrait for SocialAccountStore {
    ...
    async fn get_social_account_by_provider(&self, provider: &SocialProvider, provider_user_id: &str) -> Result<Option<SocialAccount>, AuthencError> {
        operations::social_accounts::get_social_account_by_provider(&self.database, provider, provider_user_id).await
    }
    ...
    async fn update_social_account(&self, account_id: Uuid, request: CreateSocialAccountRequest) -> Result<SocialAccount, AuthencError> {
        operations::social_accounts::update_social_account(&self.database, account_id, request).await
    }
    ...
    async fn remove_social_account_by_provider(&self, user_id: Uuid, provider: &SocialProvider) -> Result<(), AuthencError> {
        operations::social_accounts::remove_social_account_by_provider(&self.database, user_id, provider.clone()).await
    }
}