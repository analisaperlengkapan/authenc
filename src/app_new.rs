...
use crate::error::{AuthencError, Result as AuthenceResult}; // legacy placeholder
...
impl ApplicationBuilder {
    ...
    // BUILD AND RUN METHODS TEMPORARILY DISABLED DUE TO TYPE COMPLEXITY
    // Focus on fixing other compilation issues first
    
    /// Run the application server - DISABLED
    pub async fn run(self) -> std::io::Result<()> {
        panic!("ApplicationBuilder::run temporarily disabled");
    }
}
