...
pub struct DefaultEventProvider {
    persistence: Option<Arc<dyn crate::spi::store_traits::EventPersistenceProvider>>,
...
impl DefaultEventProvider {
    /// Create a new default event provider with persistence
    pub fn new(persistence: Arc<dyn crate::spi::store_traits::EventPersistenceProvider>) -> Self {
        Self {
    ...
    /// Create a new default event provider without persistence (for testing)
    ...
    /// Create a new default event provider without database (for testing) - alias
    ...
    fn convert_event_type(spi_type: EventType) -> authenc_models::models::events::EventType {
        use authenc_models::models::events::EventType as ModelEventType;
        match spi_type {
    ...
    fn convert_model_event_type(model_type: authenc_models::models::events::EventType) -> EventType {
        use authenc_models::models::events::EventType as ModelEventType;
        match model_type {
    ...
    ) -> authenc_models::models::events::OperationType {
        use authenc_models::models::events::OperationType as ModelOperationType;
        match spi_type {
    ...
    fn convert_model_admin_operation_type(
        model_type: authenc_models::models::events::OperationType,
...
impl EventStoreProvider for DefaultEventProvider {
    async fn store_event(&self, event: Event) -> Result<(), EventError> {
        if let Some(persistence) = &self.persistence {
            ...
            let model_event = authenc_models::models::events::Event {
                id: event.id.clone(),
                time: event.time,
            ...
            persistence.store_event(&model_event)
                .await
    ...
    async fn store_admin_event(&self, event: AdminEvent) -> Result<(), EventError> {
        if let Some(persistence) = &self.persistence {
            ...
            let model_event = authenc_models::models::events::AdminEvent {
                id: event.id.clone(),
                time: event.time,
                realm_id: event.realm_id.clone(),
                realm_name: None,
                auth_details: authenc_models::models::events::AuthDetails {
                ...
                resource_type: authenc_models::models::events::ResourceType::Custom,
            ...
            persistence.store_admin_event(&model_event)
                .await
                .map_err(|e| EventError::StorageError(e.to_string()))?;
        }
        Ok(())
    ...
    async fn query_events(&self, query: EventQuery) -> Result<Vec<Event>, EventError> {
        if let Some(persistence) = &self.persistence {
            let model_events = persistence.query_events(&query)
                .await
                .map_err(|e| EventError::StorageError(e.to_string()))?;
    ...
    async fn query_admin_events(
        ...
        if let Some(persistence) = &self.persistence {
            let model_events = persistence.query_admin_events(&query)
                .await
                .map_err(|e| EventError::StorageError(e.to_string()))?;
