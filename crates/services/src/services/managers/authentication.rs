...
impl DefaultAuthenticationManager {
    ...
    pub fn new(database: Arc<authenc_db::database::Database>) -> Self {
        Self {
...
impl AuthenticationManager for DefaultAuthenticationManager {
    ...
    async fn authenticate_user(
        ...
        let user_opt =
            authenc_db::database::operations::users::get_user_by_username(&self.database, &realm_uuid, username)
                .await?;
        ...
        let (success, error_message) = if let Some(ref u) = user_opt {
            if !u.enabled {
                ...
                match authenc_core::utils::crypto::password::verify_password(hash, password).await {
                    Ok(true) => (true, None),
    ...
    async fn process_flow(
        ...
        let user_opt = if let Some(ref user_id_str) = session.user_id {
            ...
            if let Ok(user_id) = uuid::Uuid::parse_str(user_id_str) {
                authenc_db::database::operations::users::get_user_by_id(&self.database, user_id).await?
            } else {
                None
            }
        } else {
