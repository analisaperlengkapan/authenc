...
impl GDPRComplianceChecks {
    ...
    pub fn data_encryption_check_with_database(
        database: Arc<authenc_db::database::Database>,
...
pub struct GDPRDataEncryptionCheck {
    requirement: ComplianceRequirement,
    database: Option<Arc<authenc_db::database::Database>>,
}
...
impl ComplianceCheck for GDPRDataEncryptionCheck {
    async fn execute(&self) -> Result<ComplianceCheckResult> {
        ...
        if let Some(db) = &self.database {
            ...
            match authenc_db::database::operations::realms::list_realms(db).await {
            ...
            match authenc_db::database::operations::users::get_all_users(db).await {
...
impl ComplianceCheck for GDPRDataRetentionCheck {
    async fn execute(&self) -> Result<ComplianceCheckResult> {
        ...
        if let Some(db) = &self.database {
            ...
            match authenc_db::database::operations::audit::get_audit_log_count(db, None, None, None)
                .await
            {
                Ok(total_count) => {
        ...
        if let Some(db) = &self.database {
            ...
            match authenc_db::database::operations::realms::list_realms(db).await {
                Ok(realms) if !realms.is_empty() => {
                    ...
                    match authenc_db::database::operations::roles::list_roles_by_realm(db, &realm_id)
                        .await
                    {
                ...
                Ok(users) if !users.is_empty() => {
                    ...
                    for user in users.iter().take(5) {
                        if let Ok(user_roles) =
                            authenc_db::database::operations::roles::get_user_roles(db, &user.id).await
                            && !user_roles.is_empty() {
                                users_with_roles += 1;
...
impl ComplianceCheck for HIPAAAuditControlsCheck {
    async fn execute(&self) -> Result<ComplianceCheckResult> {
        ...
        if let Some(db) = &self.database {
            ...
            match authenc_db::database::operations::audit::get_audit_logs(db, None, None, None, 100, 0)
                .await
            {
