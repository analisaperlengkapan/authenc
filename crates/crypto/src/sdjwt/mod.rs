...
impl IssuerSignedJwt {
    ...
    pub fn get_payload_as_value(&self) -> Result<Value, AuthencError> {
        ...
        if let Some(payload_obj) = full_payload.as_object_mut() {
            let map: &mut Map<String, Value> = payload_obj;
            for (k, v) in &self.array_claims {
                ...
                let key = k.clone();
                let _ = map.insert(key, array_value);
            }
        }
    ...
    pub fn from_string(sd_jwt_str: &str) -> Result<Self, AuthencError> {
        ...
        for (key, value) in raw_payload {
            ...
            if val.is_array() {
                ...
                if let Ok(elements) =
                    serde_json::from_value::<Vec<SdJwtArrayElement>>(val.clone())
                {
                    ...
                    if let Ok(claim) = serde_json::from_value::<SdJwtClaim>(val) {
