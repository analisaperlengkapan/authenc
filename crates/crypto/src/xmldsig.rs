                ...
                Ok(Event::Start(ref e)) => {
                    ...
                    match name.as_str() {
                        ...
                        "CanonicalizationMethod" | "ds:CanonicalizationMethod"
                            if in_signed_info =>
                        {
                            if let Some(Ok(attr)) = e.attributes().find(|res| {
                                match res {
                                    Ok(attr) => attr.key.as_ref() == b"Algorithm",
                                    Err(_) => false,
                                }
                            }) {
                        ...
                        "Reference" | "ds:Reference" if in_signed_info => {
                            ...
                            if let Some(Ok(attr)) = e.attributes().find(|res| {
                                match res {
                                    Ok(attr) => attr.key.as_ref() == b"URI",
                                    Err(_) => false,
                                }
                            }) {
                                current_ref.uri = String::from_utf8_lossy(&attr.value).to_string();
                            }
                        }
                        "DigestMethod" | "ds:DigestMethod" if in_reference => {
                            if let Some(Ok(attr)) = e.attributes().find(|res| {
                                ...
                                digest_method_uri = String::from_utf8_lossy(&attr.value).to_string();
                            }
                        }
...
impl CertificateValidator {
    ...
    pub fn validate_certificate_chain(&self, cert: &X509) -> Result<()> {
        ...
        match context.init(&self.trust_store, cert, &chain, |ctx: &mut X509StoreContextRef| ctx.verify_cert()) {
            Ok(true) => Ok(()),
            Ok(false) => {
    ...
    pub fn get_issuer_from_chain(&self, cert: &X509) -> Result<Option<X509>> {
        ...
        let issuer = context
            .init(&self.trust_store, cert, &chain, |ctx: &mut X509StoreContextRef| {
