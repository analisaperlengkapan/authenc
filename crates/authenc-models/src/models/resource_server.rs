...
impl TryFrom<tokio_postgres::Row> for ResourceServer {
    ...
    fn try_from(row: tokio_postgres::Row) -> Result<Self, Self::Error> {
        ...
        let decision_strat_str: String = row.try_get("decision_strategy").map_err(|e| {
            authenc_core::error::AuthencError::database(format!("Failed to get decision_strategy: {}", e))
        })?;
        ...
        Ok(ResourceServer {
            ...
            client_id: row.try_get("client_id").map_err(|e| {
                authenc_core::error::AuthencError::database(format!("Failed to get client_id: {}", e))
            })?,
            ...
            description: row.try_get("description").map_err(|e| {
                authenc_core::error::AuthencError::database(format!("Failed to get description: {}", e))
            })?,
