...
use authenc_database::database::operations::oauth2_providers;
...
impl PgSocialStateStore {
    ...
    async fn get_provider_config_id(&self, provider_alias: &str) -> Result<Option<Uuid>> {
        ...
        let row = self.db.query_opt(query, &[&provider_alias]).await.map_err(|e| {
            authenc_core::error::AuthencError::database(format!("Failed to look up provider config: {}", e))
        })?;
...
impl SocialStateStore for PgSocialStateStore {
    async fn create_state(
        ...
        let provider_config_id = self.get_provider_config_id(provider).await?
            .ok_or_else(|| authenc_core::error::AuthencError::ConfigurationError { message: format!("Provider {} not configured in database", provider) })?;
        ...
        let target_realm_id: Uuid = if let Some(rid_str) = realm_id {
            Uuid::parse_str(rid_str).map_err(|_| authenc_core::error::AuthencError::validation("Invalid realm_id format"))?
        } else {
            ...
            self.db.query_one::<tokio_postgres::Row>(realm_id_query, &[&provider_config_id]).await
                .map_err(|e| authenc_core::error::AuthencError::database(format!("Failed to get realm for provider: {}", e)))?
                .get(0)
        };
    ...
    async fn validate_and_consume_state(&self, state: &str) -> Result<Option<SocialLoginState>> {
        ...
        let state_data = oauth2_providers::validate_oauth2_state(&self.db, state).await
            .map_err(|e| authenc_core::error::AuthencError::database(format!("Failed to validate state: {}", e)))?;
        ...
        if let Some(data) = state_data {
            ...
            let provider_id = data["provider_config_id"]
                .as_str()
                .ok_or_else(|| authenc_core::error::AuthencError::database("Missing provider_config_id in state data"))?;

            let provider_id_uuid = Uuid::parse_str(provider_id)
                .map_err(|_| authenc_core::error::AuthencError::database("Invalid provider_config_id format"))?;
            ...
            let provider_alias: String = self.db.query_one::<tokio_postgres::Row>(query, &[&provider_id_uuid]).await
                .map_err(|e| authenc_core::error::AuthencError::database(format!("Failed to get provider alias: {}", e)))?
                .get(0);
    ...
    async fn cleanup_expired(&self) -> Result<()> {
        ...
        self.db.execute(query, &[]).await.map_err(|e| {
            authenc_core::error::AuthencError::database(format!("Failed to cleanup expired states: {}", e))
        })?;
        Ok(())
    }
}
