...
use authenc_spi::spi::authenticator::{AuthenticationResult, AuthenticatorProvider, AuthenticatorType};
...
impl DefaultAuthenticationManager {
    ...
    pub fn new(database: Arc<authenc_database::database::Database>) -> Self {
        Self {
...
impl AuthenticationManager for DefaultAuthenticationManager {
    ...
    async fn authenticate_user(
        ...
        let user_opt =
            authenc_database::database::operations::users::get_user_by_username(&self.database, &realm_uuid, username)
                .await?;
    ...
    async fn process_flow(
        ...
        let user_opt = if let Some(ref user_id_str) = session.user_id {
            ...
            if let Ok(user_id) = uuid::Uuid::parse_str(user_id_str) {
                authenc_database::database::operations::users::get_user_by_id(&self.database, user_id).await?
            } else {
                None
            }
        } else {
