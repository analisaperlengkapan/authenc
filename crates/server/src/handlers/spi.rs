...
pub async fn list_spi_providers(
    ...
    let overrides = authenc_db::database::operations::spi::get_all_provider_configs(
        &state.database,
        &spi_name,
    )
    .await
    ...
    let map_provider = |provider: &authenc_core::config::SpiProviderConfig, name_prefix: &str| {
...
pub async fn get_provider_config(
    ...
    if let Ok(Some((config, _))) = authenc_db::database::operations::spi::get_provider_config(
        &state.database,
        &spi_name,
        &provider_id,
    )
...
pub async fn update_provider_config(
    ...
    let db_config = authenc_db::database::operations::spi::get_provider_config(
        &state.database,
        &spi_name,
        &provider_id,
    )
    ...
    if let Err(e) = authenc_db::database::operations::spi::upsert_provider_config(
        &state.database,
        &spi_name,
        &provider_id,
        update.config.clone(),
...
fn get_static_provider_details(
    config: &authenc_core::config::AppConfig,
    spi_name: &str,
    provider_id: &str,
