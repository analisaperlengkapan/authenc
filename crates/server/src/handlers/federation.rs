...
fn construct_social_callback_uri(config: &authenc_core::config::AppConfig) -> String {
    format!(
...
fn convert_to_ldap_user_info(user: authenc_api::models::User) -> LdapUserInfo {
        ...
        .await
    {
        Ok(Some(user_info)) => {
            ...
            if let Some(realm_id) = user_info.realm_id {
                ...
                let jit_request = authenc_api::models::user::JITUserProvisioningRequest {
                ...
                match state.jit_provisioning_service.provision_user(jit_request).await {
                    Ok(response) => {
                        ...
                        // Generate JWT
                        if let Ok(token) = authenc_core::utils::crypto::jwt::generate_jwt(&user.id.to_string()) {
