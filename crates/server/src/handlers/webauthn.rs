...
pub async fn register_challenge(
    ...
    Json(request): Json<authenc_services::services::webauthn::WebAuthnRegistrationRequest>,
...
pub async fn register_verify(
    ...
    Json(response): Json<authenc_api::models::webauthn::WebauthnRegistrationResponse>,
) -> Result<Json<serde_json::Value>> {
    let username = params
        .get("username")
...
pub async fn authenticate_challenge(
    State(state): State<Arc<AppState>>,
    Json(request): Json<authenc_services::services::webauthn::WebAuthnAuthenticationRequest>,
) -> Result<Json<serde_json::Value>> {
    let webauthn_service = WebAuthnService::new(
        state.database.clone(),
        "localhost".to_string(),
...
pub async fn authenticate_verify(
    State(state): State<Arc<AppState>>,
    Query(params): Query<std::collections::HashMap<String, String>>,
    Json(response): Json<authenc_api::models::webauthn::WebauthnAuthenticationResponse>,
) -> Result<Json<serde_json::Value>> {
    let username = params
        .get("username")
        .ok_or(AuthencError::validation("Bad request: Missing username"))?;
