...
use authenc_api::models::user::{JITUserProvisioningRequest, JITUserProvisioningResponse};
...
impl AdminService for MockAdminService {
    async fn get_system_stats(
        &self,
    ) -> std::result::Result<authenc_services::services::admin::SystemStats, String> {
        ...
        match users::create_user(&self.db, &db_request).await {
            Ok(user) => {
                ...
                let (roles_result, groups_result): (
                    authenc_api::error::Result<Vec<authenc_api::models::Role>>,
                    authenc_api::error::Result<Vec<authenc_api::models::Group>>
