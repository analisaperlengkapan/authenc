...
pub struct DefaultEventProvider {
    // Database dependency removed - implementation should be in a separate crate
...
impl EventStoreProvider for DefaultEventProvider {
    async fn store_event(&self, _event: Event) -> Result<(), EventError> {
        // No-op by default in core. Concrete implementation should be provided by authenc-db/services.
        Ok(())
    }

    async fn store_admin_event(&self, _event: AdminEvent) -> Result<(), EventError> {
        // No-op
        Ok(())
    }
